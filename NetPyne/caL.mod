...
NEURON {
	...
	RANGE pbar, ical, mshift, hshift, qfact
}
...
BREAKPOINT {
    ...
    ical  = ghk(v,cali,calo) * pbar * m * m * (0.17*h + (1-0.17))    : Kasai 92, Brown 93
}
...
PROCEDURE settables( v (mV) ) {
	...
	TABLE minf, hinf, mtau DEPEND mshift, hshift
		...
		minf = 1  /  ( 1 + exp( (v+8.9-mshift) / (-6.7)) )
		hinf = 1  /  ( 1 + exp( (v+13.4-hshift) / 11.9) )
			: to match hinf data from Bell 2001, fig 2

		malpha = 0.0398 * (v+8.124) / ( exp((v+8.124)/9.005) - 1 )
		mbeta = 0.99 * exp(v/31.4)		: Kasai 1992, fig 15
		mtau = 1 / (malpha + mbeta)
}
