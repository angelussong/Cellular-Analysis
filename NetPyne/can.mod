...
NEURON {
	...
	RANGE pbar, ica, qfact, mshift, hshift
}
...
BREAKPOINT {
    ...
    ica  = ghk(v,cai,cao) * pbar * m * m * (0.21*h + (1-0.21))    : Kasai 92, Brown 93
}
...
PROCEDURE settables( v (mV) ) {
	...
	TABLE minf, hinf, mtau DEPEND mshift, hshift
		...
		minf = 1  /  ( 1 + exp( (v+8.7-mshift) / (-7.4)) )
		hinf = 1  /  ( 1 + exp( (v+74.8-hshift) / 6.5) )

		malpha = 0.03856 * (v+17.19) / ( exp((v+17.19)/15.22) - 1 )
		mbeta = 0.3842 * exp(v/23.82)		: Kasai 1992, fig 15
		mtau = 1 / (malpha + mbeta)
}
