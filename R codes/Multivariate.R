rm(list = ls())
ipsc_HETD1=read.csv("/Users/hsong1/Documents/HETD2_May16IR2e.csv",header=TRUE)
library(partykit)
library(party)
library(relaimpo)
library(mlbench)
...
x <- as.matrix(ipsc_HETD1[,1:3])
amp <- as.matrix(ipsc_HETD1[,4])
rise <- as.matrix(ipsc_HETD1[,5])
decay <- as.matrix(ipsc_HETD1[,6])
hfw <-as.matrix(ipsc_HETD1[,7])
